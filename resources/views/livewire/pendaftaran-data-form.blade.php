<div class="container container-fluid">
    @if(!$data['photo_image'] || !$data['ijazah_image'])
    <div class="alert alert-danger">
        @if(!$data['photo_image'])<h6 class="text-dark">Silahkan upload Photo</h6>@endif
        @if(!$data['ijazah_image'])<h6 class="text-dark">Silahkan upload Ijazah</h6>@endif
    </div>
    @endif
    <div class="p-2" style="overflow-x: auto">
        <div class="border p-2 shadow bg-white" style="width: 210mm; height: auto;">
                    ...
                    <dl class="row my-2">
                        ...
                        <dt class="col-9 mb-2">
                            <ol class="list-group list-group-numbered border-0 p-0">
                                <li class="list-group-item border-0 p-0">{{$data['jurusan']}} @if($data['jurusan_id']==$data['jurusan_id_lulus']) <strong class="text-success">(DITERIMA)</strong>  @endif</li>
                                <li class="list-group-item border-0 p-0">{{$data['jurusan2']}} @if($data['jurusan_id2']==$data['jurusan_id_lulus']) <strong class="text-success">(DITERIMA)</strong>  @endif</li>
                                <li class="list-group-item border-0 p-0">{{$data['jurusan3']}} @if($data['jurusan_id3']==$data['jurusan_id_lulus']) <strong class="text-success">(DITERIMA)</strong>  @endif</li>
                            </ol>
                        </dt>
                        ...
                        <dt class="col-12 text-primary  my-2"><i class="bi bi-check-square"></i> <strong>Data tersebut diatas adalah BENAR, Saya akan menanggung RESIKO apabila data diatas adalah SALAH</strong></dt>
                    </dl>
                </div>
            </div>
            <div class="row mb-3">
    ...
    <div class="row mb-3">
        <div class="col">
            <h6>Lampiran Ijazah</h6>
            @if($data['ijazah_image'])
            <img class="img-fluid border p-2" src="{{url('/ijazah/'.$data['ijazah_image'])}}" alt="{{$data['ijazah_image']}}"
                    style="image-position: center center; max-height: 300px; max-width: 95%">
            @else
            <h6 class="text-danger">Silahkan upload Ijazah</h6>
            @endif
        </div>
        <div class="col">
            <h6>Lampiran Transkip</h6>
            @if($data['transkip_image'])
            <img class="img-fluid border p-2" src="{{url('/transkip/'.$data['transkip_image'])}}" alt="{{$data['transkip_image']}}"
                    style="image-position: center center; max-height: 300px; max-width: 95%">
            @else
            <h6 class="text-danger">Silahkan upload Transkip</h6>
            @endif
        </div>
    </div>
</div>